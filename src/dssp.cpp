...
void Dssp::initOptions(IOptionsContainer* options, TrajectoryAnalysisSettings* settings)
{
    static const char* const desc[] = {
        ...
        "[TT]-polypro[tt] allows not to perform a search for polyproline helices. The results "
        "obtained in this way will be identical to the results of the DSSP v.2 algorithm.[PAR]"
    ...
    options->addOption(BooleanOption("polypro")
                           .store(&searchPolyPro_)
                           .defaultValue(true)
                           .description("Perform a search for polyproline helices"));
...
{
    return gmx::TrajectoryAnalysisCommandLineRunner::runAsMain<Dssp>(argc, argv);
}
